...
func buildChart(snaps []Snapshot, emailToName map[string]string) chartData {
	// Rank authors by total lines-across-time (proxy for importance).
	...
	ds := make([]chartDataset, 0, len(authors))
	for i, author := range authors {
		color := palette[i%len(palette)]
		pct := make([]float64, len(snaps))
		abs := make([]float64, len(snaps))
		for j, s := range snaps {
			abs[j] = float64(s.Totals[author])
			if s.Total > 0 {
				pct[j] = float64(s.Totals[author]) / float64(s.Total) * 100
			}
		}
		// Prefer the real display name; fall back to the local part of the email.
		label := emailToName[author]
		if label == "" {
			label = author
			if at := strings.LastIndex(author, "@"); at >= 0 {
				label = author[:at]
			}
		}
...
`)
	...
	// 4. Build chart data.
	cd := buildChart(snaps, emailToName)
